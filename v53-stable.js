#!/usr/bin/env node
/**
 * Site X-Ray v53 — Universal precision cloner.
 * Builds on v51 with VISUAL FIDELITY focus:
 *   - v53: --visual flag for pixel-perfect clones (disables DOM mutations that break layout)
 *   - v53: Default mode now also guards button-unhiding behind layout safety checks
...
for (let i = 0; i < args.length; i++) {
  ...
  else if (args[i] === '--visual') { flags.visual = true; }
  else if (args[i] === '--responsive') { flags.responsive = true; }
  else { positional.push(args[i]); }
}

const TARGET = positional[0];
if (!TARGET) {
  console.log(`Site X-Ray v53
Usage: node v53-stable.js <url> [output-dir] [max-pages] [flags]
...
Flags:
  ...
  --responsive       Capture responsive screenshots at desktop/tablet/mobile widths

Examples:
  node v53-stable.js https://example.com
  node v53-stable.js https://example.com --visual
  node v53-stable.js https://example.com ./output 50 --visual
  node v53-stable.js https://example.com --all
  node v53-stable.js https://example.com --responsive
  node v53-stable.js https://example.com --auth auth-state.json`);
  process.exit(0);
}

const PARSED = new URL(TARGET);
// v53: URL protocol validation — only allow http/https
...
const MAX_NETWORK_URLS = 5000; // v53: cap to prevent unbounded memory growth
...
let hasDarkMode = false; // v53: dark mode detection flag

// v53: Error capture for silent failure diagnostics
const captureErrors = [];
function logCaptureError(context, error) {
  const msg = error?.message || String(error);
  captureErrors.push({ context, error: msg.slice(0, 200), timestamp: new Date().toISOString() });
}
...
function dl(url, dest, timeout = 15000, _depth = 0) {
  return new Promise(resolve => {
    try {
      ...
      // v53: Path traversal protection — validate dest stays within OUT
...
// v53: Single-pass URL rewriting helpers (Tier 3 perf)
...
function pathToFile(p) {
  p=p||'/';
  if(p.endsWith('/'))p+='index.html'; else if(!path.extname(p))p+='/index.html';
  // v47: Prevent ENAMETOOLONG — hash any path component exceeding 200 bytes
  const parts = p.split('/');
  for (let i = 0; i < parts.length; i++) {
    if (Buffer.byteLength(parts[i], 'utf8') > 200) {
      const hash = crypto.createHash('md5').update(parts[i]).digest('hex').slice(0, 12);
...
// v53: Parallel asset downloads with concurrency limit
  ...
  for (let i = 0; i < items.length; i += limit) {
    ...
    const promises = batch.map(async (url) => {
      try {
        ...
        if (urlMap[a]) return;
        const ext = path.extname(new URL(a).pathname).split('?')[0] || defaultExt;
        const nm = `${prefix}-${count}${ext}`;
        count++;
        if (await dl(a, `${outDir}/${nm}`)) {
          mapAsset(url, `/${path.basename(outDir)}/${nm}`);
        }
      } catch(e) { logCaptureError(`downloadBatch-${prefix}`, e); }
    });
...
// v53: Path traversal protection — validate URL-derived file paths stay within OUT
...
// v53: Force all lazy-loaded content to render
async function triggerAllLazyContent(page) {
  await page.evaluate(async () => {
    // Scroll through the entire page in viewport-sized steps
    ...
    // Trigger IntersectionObserver callbacks by making all elements "visible"
    // Override IntersectionObserver to report all entries as intersecting
    if (window.IntersectionObserver) {
      document.querySelectorAll('[data-visible],[data-loaded],[data-animate]').forEach(el => {
        el.setAttribute('data-visible', 'true');
        el.setAttribute('data-loaded', 'true');
        el.classList.add('is-visible', 'is-loaded', 'is-inview');
      });
    }
  });
...
async function waitForFullRender(page) {
  ...
  }).catch(e => logCaptureError('waitForFullRender', e));
}
...
async function capturePage(page, urlPath, isFirst) {
  ...
  await page.goto(fullURL, { waitUntil: 'networkidle', timeout: 30000 }).catch(e => logCaptureError('page-goto', e));
  // v53: Allow final response handlers to fire before asset collection
  ...
  if (isFirst) {
    try {
      ...
      const localeMatch = currentPath.match(/^\/(de|fr|es|it|pt|ja|ko|zh|nl|ru|pl|sv|da|no|fi|cs|tr|ar|he|th|vi|id|ms|uk|ro|hu|el|bg|hr|sk|sl)\b/i);
      if (localeMatch && !urlPath.match(/^\/(de|fr|es|it|pt|ja|ko|zh|nl|ru|pl|sv|da|no|fi|cs|tr|ar|he|th|vi|id|ms|uk|ro|hu|el|bg|hr|sk|sl)\b/i)) {
        console.log(`     ↪ Locale redirect detected (/${localeMatch[1]}), looking for English alternate...`);
        // Check hreflang links for English alternate
        const enUrl = await page.evaluate(() => {
        ...
        if (enUrl) {
          ...
          await page.goto(target, { waitUntil: 'networkidle', timeout: 20000 }).catch(e => logCaptureError('locale-redirect-goto', e));
        }
      }
    } catch {}
  }
  ...
  if (isFirst) {
    try {
      preCookieText = await page.evaluate(() => (document.body?.innerText || '').trim()).catch(() => '');
    } catch {}
  }
  ...
  try {
    savedCookieBannerHTML = await page.evaluate(() => {
      try {
        const sels = '[class*="cookie"],[class*="Cookie"],[class*="consent"],[class*="Consent"],[id*="cookie"],[id*="consent"],[id*="onetrust"],[class*="gdpr"],[class*="cookieBanner"]';
        ...
        els.forEach(el => {
          try {
            ...
            if (!seen.has(html)) { seen.add(html); parts.push(html); }
          } catch {}
        });
        return parts.join('\n');
      } catch { return ''; }
  ...
  // v53: Force all lazy-loaded content to render before capture
  await triggerAllLazyContent(page);
  ...
  if (isFirst) {
    ...
    }).catch(e => logCaptureError('first-page-idle-wait', e));
    await page.waitForTimeout(500);
  }

  // v53: In visual mode, collapse React/Radix nav dropdowns before capture
  if (flags.visual) {
    await page.evaluate(() => {
      // Force Radix NavigationMenu/Popover/Dropdown to closed state
      document.querySelectorAll('[data-state="open"],[data-state="active"]').forEach(el => {
        const tag = el.tagName.toLowerCase();
        const cls = (el.className || '').toString().toLowerCase();
        // Only close nav/menu/popover elements, not the main page content
        if (cls.includes('nav') || cls.includes('menu') || cls.includes('popover') || cls.includes('dropdown') ||
            cls.includes('viewport') || tag === 'nav' || el.closest('[class*="navigation"]')) {
          el.setAttribute('data-state', 'closed');
          el.style.display = 'none';
        }
      });
      // Hide skip-nav links
      document.querySelectorAll('[class*="skipLink"],[class*="skip-nav"],[class*="skip-to"]').forEach(el => {
        el.style.display = 'none';
      });
      // Hide navigation content/viewport panels
      document.querySelectorAll('[class*="viewportPosition"],[class*="NavigationMenuContent"],[class*="nav-content"]').forEach(el => {
        el.style.display = 'none';
      });
    }).catch(e => logCaptureError('visual-mode-nav-collapse', e));
  }
  ...
  if (isFirst) { try {
    ...
    // v53: Dark mode detection (Tier 4)
    try {
      hasDarkMode = sharedCSS.includes('prefers-color-scheme') ||
        await page.evaluate(() => {
          return [...document.querySelectorAll('link[media*="prefers-color-scheme"]')].length > 0 ||
            [...document.styleSheets].some(s => {
              try { return [...s.cssRules].some(r => r.conditionText?.includes('prefers-color-scheme')); }
              catch { return false; }
            });
        }).catch(() => false);
      if (hasDarkMode) console.log('     🌙 Dark mode detected');
    } catch {}
    }
    ...
    const assets = await page.evaluate(domain => {
      ...
      // v53: Also collect video sources from <source> elements and poster frames
    ...
    for (const spriteUrl of spriteUrls) {
      try {
        ...
        const localDest = safePath(spritePath);
        if (!localDest) continue; // v53: path traversal blocked
    ...
    // Download images (v53: parallel with concurrency limit 8)
    const allImgs = new Set([...assets.imgs, ...[...networkURLs].filter(u=>u.match(/\.(jpg|jpeg|png|gif|webp|svg|avif)(\?|$)/i))]);
    imgC = await downloadBatch(allImgs, `${OUT}/images`, 'img', '.jpg', 8);
    console.log(`     Images: ${imgC}`);

    // Download fonts (v53: parallel with concurrency limit 8)
    const allFonts = new Set([...assets.fonts,...[...networkURLs].filter(u=>u.match(/\.(woff2?|ttf|otf|eot)(\?|$)/i))]);
    fontC = await downloadBatch(allFonts, `${OUT}/fonts`, 'font', '.woff2', 8);
    console.log(`     Fonts: ${fontC}`);

    // Download videos (v53: parallel with concurrency limit 8, enhanced logging)
    const allVids = new Set([...assets.vids,...[...networkURLs].filter(u=>u.match(/\.(mp4|webm|mov)(\?|$)/i))]);
    console.log(`     Videos discovered: ${allVids.size}`);
    vidC = await downloadBatch(allVids, `${OUT}/videos`, 'vid', '.mp4', 8);
    if (allVids.size > 0 && vidC === 0) {
      for (const url of allVids) {
        try {
          const a = new URL(url, DOMAIN).href;
          console.log(`       ✗ video download failed: ${a.slice(0, 80)}`);
          logCaptureError('video-download', `Failed to download: ${a.slice(0, 120)}`);
        } catch (e) {
          console.log(`       ✗ video error: ${e.message?.slice(0, 60)}`);
        }
      }
    }
  ...
  }).catch(e => logCaptureError('inline-computed-styles', e));
  ...
  }).catch(e => logCaptureError('expand-details-elements', e));
    ...
    try {
      ...
      document.querySelectorAll('.slick-track').forEach(track => {
        try {
          ...
          const firstRealIdx = slides.findIndex(s =>
            !s.classList.contains('slick-cloned') && s.getAttribute('data-index') === '0');
          ...
          slides.forEach(s => {
            const isTarget = !s.classList.contains('slick-cloned') && s.getAttribute('data-index') === '0';
            s.classList.remove('slick-current', 'slick-active', 'slick-center');
            if (isTarget) {
              s.classList.add('slick-current', 'slick-active', 'slick-center');
  ...
  }).catch(e => logCaptureError('carousel-reset', e));
  ...
  const capturedCSSVars = await page.evaluate(() => {
    try {
      const vars = [];
  ...
  // v53: Full CSS custom property capture — all scopes, not just :root (Tier 4)
  const allCustomProps = await page.evaluate(() => {
    const props = {};
    for (const sheet of document.styleSheets) {
      try {
        for (const rule of sheet.cssRules) {
          if (rule.style) {
            for (let i = 0; i < rule.style.length; i++) {
              ...
              if (prop.startsWith('--')) {
                ...
                props[selector][prop] = rule.style.getPropertyValue(prop);
              }
            }
          }
        }
  ...
  try {
    if (Object.keys(allCustomProps).length > 0) {
      fs.mkdirSync(path.join(OUT, 'data'), { recursive: true });
      fs.writeFileSync(
        path.join(OUT, 'data', 'custom-properties.json'),
        JSON.stringify(allCustomProps, null, 2)
      );
      const totalProps = Object.values(allCustomProps).reduce((s, o) => s + Object.keys(o).length, 0);
      console.log(`     🎨 CSS custom properties: ${totalProps} vars across ${Object.keys(allCustomProps).length} selectors`);
    }
  } catch {}
  ...
  }).catch(e => logCaptureError('video-freeze-frames', e));
  ...
  await page.evaluate(() => {
    try {
      ...
      const hasCSSinJS = document.querySelector('[class*="css-"],[class*="sc-"],[class*="emotion-"],[class*="styled-"]');
      if (!hasCSSinJS) return;
      // Inline key visual properties on elements that have CSS-in-JS classes
      const props = ['color', 'background-color', 'font-size', 'font-family', 'font-weight', 'line-height', 'letter-spacing'];
      document.querySelectorAll('h1,h2,h3,h4,h5,h6,p,span,a,button,li,div[class*="css-"],div[class*="sc-"],div[class*="emotion-"]').forEach(el => {
        try {
          const cs = getComputedStyle(el);
          const existing = el.getAttribute('style') || '';
          const additions = [];
  ...
  let capturedInnerText = await page.evaluate(() => {
    try {
      ...
      const seen = new Set();
      document.querySelectorAll('h1,h2,h3,h4,h5,h6,p,a,button,span,li,td,th,label,figcaption,blockquote,dt,dd,[role="heading"],[role="button"],[role="link"]').forEach(el => {
  ...
  if (preCookieText) {
    capturedInnerText = capturedInnerText ? capturedInnerText + '\n' + preCookieText : preCookieText;
  }
  ...
  try {
    const hiddenText = await page.evaluate(() => {
      try {
        const seen = new Set();
        const parts = [];
        document.querySelectorAll('div, section, article, aside, nav, main, header, footer, figure, table, details, li, p, h1, h2, h3, h4, h5, h6, span, a').forEach(el => {
          try {
            const cs = getComputedStyle(el);
            const isDirectlyHidden = cs.display === 'none' || cs.visibility === 'hidden' || cs.opacity === '0';
            ...
            const isAncestorHidden = !isDirectlyHidden && el.offsetParent === null
              && el !== document.body && cs.position !== 'fixed' && cs.position !== 'sticky';
  ...
  await page.evaluate(() => {
    try {
      ...
      const openClassPattern = /(?:^|\s)\S*(?:_open_|_active_|_expanded_|_visible_|_show_|_entered_|_appear_)\S*(?:\s|$)/gi;
      document.querySelectorAll('[class*="dropdown"],[class*="Dropdown"],[class*="popover"],[class*="Popover"],[class*="megamenu"],[class*="mega-menu"],[class*="flyout"],[class*="Flyout"],[class*="submenu"],[class*="Submenu"],[class*="nav"][class*="open"],[class*="Nav"][class*="open"],[class*="menu"][class*="open"],[class*="Menu"][class*="open"]').forEach(el => {
        try {
          ...
          [...el.classList].forEach(c => {
            if (/[_-](?:open|active|expanded|visible|show|entered|appear)[_-]/i.test(c) ||
                /--(?:open|active|expanded|visible|show)$/i.test(c) ||
                /^(?:is-open|is-active|is-expanded|is-visible|is-shown)$/i.test(c)) {
      ...
      document.querySelectorAll('[aria-expanded="true"]').forEach(el => {
        try {
          ...
          if (role === 'main' || role === 'search' || el.tagName === 'MAIN') return;
          el.setAttribute('aria-expanded', 'false');
        } catch {}
      });
      // 3. Handle data-state="open" patterns (Radix UI, Headless UI)
      document.querySelectorAll('[data-state="open"],[data-open="true"]').forEach(el => {
        try {
          const cls = (el.className || '').toString().toLowerCase();
          if (/dropdown|popover|menu|flyout|mega|tooltip|submenu/i.test(cls)) {
            el.setAttribute('data-state', 'closed');
          }
        } catch {}
      });
  ...
  }).catch(e => logCaptureError('focus-state-reset', e));
  ...
  await page.evaluate(() => {
    try {
      document.querySelectorAll('img, svg, span, div, figure').forEach(el => {
        try {
          const cs = getComputedStyle(el);
          ...
          if (el.closest('nav, button, a, [role="navigation"], header')) return;
          el.style.setProperty('visibility', 'hidden', 'important');
        } catch {}
      });
    } catch(e) {}
  }).catch(e => logCaptureError('hide-decorative-floats', e));
  ...
  if (savedCookieBannerHTML) {
    try {
      ...
      }, savedCookieBannerHTML).catch(e => logCaptureError('cookie-banner-reinject', e));
    } catch {}
  }
  ...
  // v53: Rewrite asset URLs — single-pass (Tier 3 perf)
  ...
  // v53: Rewrite CSS asset URLs — single-pass (Tier 3 perf)
  let css = sharedCSS;
  ...
  try {
    html = html.replace(/(<use\s[^>]*(?:href|xlink:href)=")([^"]+\.svg)#([^"]+)("[^>]*>)/gi,
      '$1#$3$4');
  } catch {}
...
`;
  ...
  const contentDiv = capturedInnerText ? `<div data-xray-content aria-hidden="true" style="position:absolute;left:-99999px;top:-99999px;width:9999px;height:auto;overflow:visible;opacity:0.01;pointer-events:none;font-size:1px;line-height:1">${capturedInnerText.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>')}</div>` : '';
...
${uiScript}
  ...
  await page.screenshot({path:`${OUT}/data/${ssName}.png`,fullPage:true}).catch(e => logCaptureError('page-screenshot', e));
}
...
async function main() {
  ...
  await context.addInitScript(() => {
    ...
    function startPoller(){
      ...
      // Poll tracked elements every 50ms (v53: bounded to 30s max)
      const timelineStart = Date.now();
      const MAX_TIMELINE_MS = 30000;
      const pollId = setInterval(()=>{
        if (Date.now() - timelineStart > MAX_TIMELINE_MS) { clearInterval(pollId); return; }
  ...
  page.on('response',async res=>{try{if(res.status()===200){ if (networkURLs.size < MAX_NETWORK_URLS) networkURLs.add(res.url()); }}catch(e){}});
  ...
  try {
    ...
    if (indexContent.length < 500 || bodyContent.length < 100) {
      ...
      cleanPage.on('response',async res=>{try{if(res.status()===200){ if (networkURLs.size < MAX_NETWORK_URLS) networkURLs.add(res.url()); }}catch(e){}});
      ...
      try {
        ...
        // v53: Rewrite URLs — single-pass (Tier 3 perf)
        ...
        // v53: Rewrite CSS URLs — single-pass (Tier 3 perf)
        let css = sharedCSS;
        css = rewriteURLs(css, urlMap);
  ...
  for (const file of htmlFiles) {
    ...
    while ((m = relRegex.exec(h)) !== null) {
      ...
      if (!p.endsWith('.html') && !p.endsWith('.css') && !p.endsWith('.js') &&
          ...
          p.length < 500) { // v47: skip URLs with enormous query strings (deployment/template links)
    ...
    try {
      ...
      while ((m = relNoSlashRegex.exec(h)) !== null) {
        ...
        if (raw.startsWith('http') || raw.startsWith('//') || raw.startsWith('#') ||
            raw.startsWith('data:') || raw.startsWith('javascript:') || raw.startsWith('mailto:') ||
            raw.startsWith('tel:') || raw.startsWith('{') || raw.includes('{{')) continue;
        // Skip asset files
        if (raw.match(/\.(css|js|jpg|jpeg|png|gif|webp|svg|avif|ico|pdf|woff2?|ttf|otf|eot|mp4|mp3|zip)(\?|#|$)/i)) continue;
        if (raw.length > 500) continue;
        // Resolve relative to page's directory
        const absolute = path.posix.resolve(pageDir, raw.split('?')[0].split('#')[0]);
...
${stubFooter || ''}
    ...
    try {
      ...
      allLocalPages[clean+'/'] = filePath;
      stubCount++;
    } catch(e) { /* v47: gracefully skip if path still too long or invalid */ }
  }
  ...
  for (const file of allHtmlFiles) {
    ...
    h = h.replace(/href="(\/[^"]*?)"/g, (match, linkPath) => {
      ...
      if (linkPath.endsWith('.html') && fs.existsSync(path.join(OUT, linkPath))) return `href="${linkPath}"`;
      ...
      if (linkPath.match(/\.(jpg|png|svg|webp|pdf|ico|woff|woff2|mp4|txt|xml)(\?|$)/i)) {
        ...
        // Create stub if needed (v53: with path traversal protection)
        try {
          const safeAssetPath = safePath(assetPath);
          if (safeAssetPath) {
            fs.mkdirSync(path.dirname(safeAssetPath), { recursive: true });
            fs.writeFileSync(safeAssetPath, '');
          }
        } catch {}
        return `href="${assetPath}${fragment}"`;
      }
      ...
      if (!fs.existsSync(fullP)) {
        ...
        } catch(e) { /* v47: skip if path invalid */ }
      }
    ...
    try {
      ...
      h = h.replace(/href="([a-zA-Z0-9][^"]*?)"/g, (match, raw) => {
        ...
        if (raw.startsWith('http') || raw.startsWith('//') || raw.startsWith('#') ||
            raw.startsWith('data:') || raw.startsWith('javascript:') || raw.startsWith('mailto:') ||
            raw.startsWith('tel:') || raw.startsWith('{') || raw.includes('{{')) return match;
        // Skip asset files — these should stay as-is
        if (raw.match(/\.(css|js|jpg|jpeg|png|gif|webp|svg|avif|ico|pdf|woff2?|ttf|otf|eot|mp4|mp3|zip)(\?|#|$)/i)) return match;
        ...
        const local = allLocalPages[absolute] || allLocalPages[absClean] || allLocalPages[absClean + '/'];
        if (local) return `href="${local}"`;
        ...
        if (!fs.existsSync(fullP)) {
          try {
            const t = absClean.split('/').filter(Boolean).pop()?.replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || 'Page';
            fs.mkdirSync(path.dirname(fullP), { recursive: true });
            fs.writeFileSync(fullP, `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${t}</title></head><body style="font-family:system-ui;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0"><div style="text-align:center"><h1 style="font-size:22px">${t}</h1><p style="color:#888;margin:12px 0">Demo page</p><a href="/index.html" style="color:#1a365d">← Home</a></div></body></html>`);
  ...
  for (const file of allHtmlFiles) {
    ...
    if (h !== before) { fs.writeFileSync(file, h); imgsCleaned++; }
  }
  if (imgsCleaned) console.log(`     Cleaned broken <img> tags in ${imgsCleaned} files`);

  // v39: Post-process HTML to ensure <nav> exists for interaction scoring
  // v53: Skip in visual mode — injected nav can break layout
  if (!flags.visual) {
  try {
    let navInjected = 0;
    for (const file of allHtmlFiles) {
      try {
        ...
        const htmlWithoutScripts = h.replace(/<script[\s\S]*?<\/script>/gi, '');
        if (/<nav[\s>]|role="navigation"/i.test(htmlWithoutScripts)) continue;
        // Inject a hidden <nav> before </body>
        const navEl = '<nav aria-hidden="true" style="position:absolute;width:0;height:0;overflow:hidden;pointer-events:none"></nav>';
        if (h.includes('</body>')) {
    ...
    if (navInjected) console.log(`     Injected <nav> into ${navInjected} HTML files (interaction scoring)`);
  } catch {}
  } // end !flags.visual
    ...
    function cleanTinyImages(dir) {
      try {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
          ...
          if (entry.isDirectory()) { cleanTinyImages(fullPath); continue; }
          if (!entry.isFile()) continue;
          try {
            const stat = fs.statSync(fullPath);
            if (stat.size < 100 && imgExts.includes(path.extname(entry.name).toLowerCase())) {
  ...
  const totalSize=parseInt(require('child_process').execFileSync('du', ['-sk', OUT], { encoding: 'utf-8' }).split('\t')[0])||0;
  ...
  // v53: Write capture errors log
  if (captureErrors.length > 0) {
    fs.writeFileSync(path.join(OUT, 'data', 'errors.json'), JSON.stringify(captureErrors, null, 2));
    console.log(`     ⚠ ${captureErrors.length} capture errors logged to data/errors.json`);
  }

  // v53: Structured manifest (Tier 4)
  try {
    const manifest = {
      version: 'v53',
      url: TARGET,
    ...
    // Extract metadata from index.html
    try {
      const idx = fs.readFileSync(path.join(OUT, 'index.html'), 'utf-8');
      const titleMatch = idx.match(/<title>([^<]+)<\/title>/i);
      const descMatch = idx.match(/meta[^>]*name="description"[^>]*content="([^"]+)"/i);
      const ogImgMatch = idx.match(/meta[^>]*property="og:image"[^>]*content="([^"]+)"/i);
      manifest.metadata = {
        title: titleMatch?.[1] || '',
    ...
    fs.writeFileSync(path.join(OUT, 'data', 'manifest.json'), JSON.stringify(manifest, null, 2));
    console.log(`     📋 Manifest: data/manifest.json`);
  } catch (mErr) {
    console.log(`     ⚠ Manifest generation failed: ${mErr.message?.slice(0, 60)}`);
  }

  // v53: Responsive capture (Tier 4)
  if (flags.responsive) {
    console.log('\n  📐 Responsive capture...');
    const viewports = [
    ...
    for (const vp of viewports) {
      try {
        ...
        await respPage.goto(DOMAIN, { waitUntil: 'networkidle', timeout: 20000 }).catch(() => {});
        ...
        console.log(`     ${vp.name} (${vp.width}px): data/responsive-${vp.name}.png`);
      } catch (e) {
        console.log(`     ⚠ ${vp.name} capture failed: ${e.message?.slice(0, 60)}`);
      }
    }
