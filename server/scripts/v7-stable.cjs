...
function dl(url, dest, timeout = 8000) {
...
async function capturePage(page, urlPath, isFirst) {
  ...
  if (isFirst) {
    ...
    console.log(`     Downloading ${allImgs.size} images...`);
    let imgI=0; for(const url of allImgs){imgI++;try{const a=new URL(url,DOMAIN).href;const ext=path.extname(new URL(a).pathname).split('?')[0]||'.jpg';const nm=`img-${imgC}${ext}`;if(await dl(a,`${OUT}/images/${nm}`)){mapAsset(url,`/images/${nm}`);imgC++;if(imgC%10===0)console.log(`       ${imgC}/${allImgs.size} images...`);}}catch(e){}}
    console.log(`     Images: ${imgC}`);

    // Download fonts
    const allFonts = new Set([...assets.fonts,...[...networkURLs].filter(u=>u.match(/\.(woff2?|ttf|otf|eot)(\?|$)/i))]);
    console.log(`     Downloading ${allFonts.size} fonts...`);
    ...
    if(allVids.size)console.log(`     Downloading ${allVids.size} videos...`);
