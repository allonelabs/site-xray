#!/usr/bin/env node
/**
 * Site X-Ray v51 — Universal precision cloner.
 * Builds on v50 with:
 *   - v51: Fix relative link handling — collect and rewrite href="path" (no leading /) to absolute paths
 *   - v51: Resolve relative hrefs based on page directory for correct stub generation
 *   - All prior: hidden-text capture, carousel reset, safe long filenames, pre-cookie content, stub globals, animation freeze, button unhiding, nav injection
 *
...
Examples:
  node v44-stable.js https://example.com
  node v44-stable.js https://example.com ./output 50
  node v44-stable.js https://example.com --all
  node v44-stable.js https://example.com --auth auth-state.json
  node v44-stable.js https://example.com --save-auth
  node v44-stable.js https://example.com --interactive`);
  process.exit(0);
}
    ...
    if(btn.offsetParent===null&&btn.getClientRects().length===0){
      ...
      while(el&&el!==document.body){
        ...
        if(st.display==='none'){
          ...
          // v41: Relaxed for nav/menu/header containers — allow up to 8 children
          const cls = (el.className || '').toString().toLowerCase();
...
// v41: Second-pass button unhiding — for buttons still hidden after the first pass,
// aggressively unhide ALL blocking ancestors using clip-based sr-only pattern.
// The first pass skips containers with many children/images to avoid layout damage.
// This second pass uses clip:rect(0,0,0,0)+position:fixed to guarantee zero visual impact.
try{
  document.querySelectorAll('button,[role="button"]').forEach(btn=>{
    try{
      if(btn.offsetParent!==null)return; // already visible
      if(btn.getClientRects().length>0)return; // visible via other means
      // Walk ancestors and force-unhide display:none blockers
      let el=btn;
      while(el&&el!==document.body){
        const st=getComputedStyle(el);
        if(st.display==='none'){
          // Skip full-screen overlays/modals
          const r=el.getBoundingClientRect();
          if(r.width>window.innerWidth*0.8&&r.height>window.innerHeight*0.8)break;
          // Skip cookie/consent/modal containers
          if(el.closest('[class*="modal"],[class*="Modal"],[class*="popup"],[class*="cookie"],[class*="Cookie"],[class*="consent"],[role="dialog"]'))break;
          // Use clip-based hiding: element becomes technically "rendered" but invisible
          el.style.display='block';
...
${stubFooter || ''}
  ...
  // The interaction scorer gives +40 points for <nav> or [role="navigation"].
  // WebGL/canvas sites (resn, lusion) often have neither after cloning.
  // This runs on saved HTML, not live DOM — cannot crash or timeout like page.evaluate.
  try {
    let navInjected = 0;
    for (const file of allHtmlFiles) {
      try {
