...
for (let i = 0; i < args.length; i++) {
  ...
  } else if (args[i] === "--form-email") {
    flags.formEmail = args[++i];
...
# v54 self-debug flags
  ...
  --form-email <to>        For each <form method=POST> in the clone, the
                           auto-generated handler will forward submissions to
                           this email via Resend (RESEND_API_KEY env var
                           required on the deployed project).
...
// v55 P3: walk all .html in clone, extract <form action="..." method="POST">
// and the field names. Returns deduped list. Skips forms with action=""
// (current page) and external-domain actions (we can't intercept those).
function scanFormsInClone(outDir) {
  const found = new Map(); // action → { method, fields: Set, foundOnPages: [] }
  const walk = (dir) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        // Only POST forms — GET forms are typically search and work fine
        // statically (page can read ?q= from URL).
        for (const m of html.matchAll(
          /<form\b([^>]*?)\bmethod=["']?post["']?([^>]*?)>([\s\S]*?)<\/form>/gi,
        )) {
          const formTag = `<form${m[1]} method="post"${m[2]}>`;
          const actionMatch = formTag.match(/\baction=["']([^"']*)["']/i);
          const action = actionMatch ? actionMatch[1] : "";
          // Skip empty / page-relative / cross-origin
          if (!action || action.startsWith("javascript:")) continue;
          ...
          const localPath = action.startsWith("/")
            ? action
            : "/" + action.replace(/^\.\//, "");
          // Skip if collides with the API rewrites we just emitted
          if (localPath.startsWith("/data/")) continue;
          const fields = [...m[3].matchAll(/\bname=["']([^"']+)["']/gi)].map(
            (x) => x[1],
          );
          if (!found.has(localPath)) {
            found.set(localPath, {
              method: "POST",
              action: localPath,
              fields: new Set(fields),
              foundOnPages: [path.relative(outDir, fullPath)],
            });
          } else {
            const e = found.get(localPath);
            for (const f of fields) e.fields.add(f);
            if (e.foundOnPages.length < 5)
              e.foundOnPages.push(path.relative(outDir, fullPath));
          }
        }
      }
    }
...
function writeFormHandler(outDir, form, formEmail) {
  ...
  // Vercel function filename mirrors the action path; dynamic [slug] not
  // supported here — only literal paths. e.g. /contact → api/contact.js
  const safeName = form.action
    .replace(/^\//, "")
    .replace(/[^a-z0-9._-]/gi, "_")
    .replace(/^_+|_+$/g, "");
  const fnPath = path.join(apiDir, `${safeName || "form"}.js`);
  const fnBody = `// Auto-generated by site-xray v55 P3 — stub for ${form.action}
// Fields observed: ${form.fields.join(", ") || "(none parsed)"}
// Found on pages:  ${form.foundOnPages.slice(0, 3).join(", ")}${form.foundOnPages.length > 3 ? ", ..." : ""}
module.exports = async (req, res) => {
  ...
  let parsed = body;
  try {
    if ((req.headers["content-type"] || "").includes("json")) parsed = JSON.parse(body);
    else if ((req.headers["content-type"] || "").includes("urlencoded"))
      parsed = Object.fromEntries(new URLSearchParams(body));
  } catch {}
  console.log("[site-xray form]", ${JSON.stringify(form.action)}, parsed);
${
  formEmail
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "site-xray@allonelabs.com",
          to: ${JSON.stringify(formEmail)},
          subject: "Form submission: " + ${JSON.stringify(form.action)},
...
`
    : ""
}  res.status(200).json({ ok: true, received: parsed });
};
`;
  fs.writeFileSync(fnPath, fnBody);
}
...
${stubFooter || ""}
  ...
  try {
    ...
    // v55 P3: scan cloned HTML for <form action="..." method="POST"> and
    // generate a tiny Vercel function at each unique action path. Static
    // clones today have form markup but submit→404; this gives a 200 +
    // stub JSON response so submits don't break the user experience. The
    // function logs the body to stdout (visible in Vercel function logs)
    // and optionally forwards to a Resend email if --form-email <to> was
    // passed.
    try {
      const forms = scanFormsInClone(OUT);
      if (forms.length > 0) {
        for (const f of forms) writeFormHandler(OUT, f, flags.formEmail);
        console.log(
          `     📨 Forms: ${forms.length} action${forms.length === 1 ? "" : "s"} stubbed at api/* (Vercel functions)`,
        );
      }
    } catch (fErr) {
      console.log(`     ⚠ Form scan failed: ${fErr.message?.slice(0, 60)}`);
    }
  } catch (mErr) {
    console.log(
