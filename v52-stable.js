#!/usr/bin/env node
/**
 * Site X-Ray v52 — Universal precision cloner.
 * Builds on v51 with VISUAL FIDELITY focus:
 *   - v52: --visual flag for pixel-perfect clones (disables DOM mutations that break layout)
 *   - v52: Default mode now also guards button-unhiding behind layout safety checks
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--auth') { flags.auth = args[++i]; }
    ...
    if (colonIdx > 0) {
      const route = spec.slice(0, colonIdx).startsWith('/') ? spec.slice(0, colonIdx) : '/' + spec.slice(0, colonIdx);
...
if (!TARGET) {
  console.log(`Site X-Ray v52
Usage: node v52-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v52-stable.js https://example.com
  node v52-stable.js https://example.com --visual
  node v52-stable.js https://example.com ./output 50 --visual
  node v52-stable.js https://example.com --all
  node v52-stable.js https://example.com --auth auth-state.json
  node v52-stable.js https://www.kenkais.com --passcode /exclusive:5511 --passcode /agency:5511`);
  process.exit(0);
}
...
async function tryUnlockPasscode(page, urlPath) {
  ...
  const pinLocator = page.locator('input[maxlength="1"]:visible, input[inputmode="numeric"][maxlength="1"]:visible');
  ...
  if (pinCount >= code.length) {
    ...
    console.log(`     🔓 PIN ${'•'.repeat(code.length)} entered for ${urlPath}`);
  } else if (pwCount > 0) {
    mode = 'password';
    try {
      ...
      const navWait = page.waitForNavigation({ timeout: 5000, waitUntil: 'domcontentloaded' }).catch(() => null);
  ...
  // Sanity check.
  const stillLocked = (await page.locator('input[type="password"]:visible, input[maxlength="1"]:visible').count().catch(() => 0)) > 0
    && (await page.evaluate(() => /restricted access|enter.*code|password.*required/i.test(document.body.innerText || '')).catch(() => false));
  if (stillLocked) {
    console.log(`     ⚠️  ${urlPath} appears still locked — code may be wrong or gate type unsupported`);
  } else {
